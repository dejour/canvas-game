...
function updateEntities(dt) {
	...
	//update all the explosion
	for (var i = 0; i < explosion.length; i++) {
		explosion[i].sprite.update(dt);

		//remove if animation is done
		if (explosion[i].sprite.done) {
...
function checkCollisions() {
	...
	//run collisions detection for all enemies and bullets
	for (var i = 0; i < enemies.length; i++) {
		...
		for (var j = 0; j < bullets.length; j++) {
			...
			if (boxCollides(pos, size, pos2, size2)) {
				...
				explosion.push({
					pos: pos,
					sprite: new Sprite('img/sprites.png', [0, 117], [39, 39],
...
function render() {
	...
	// render player if the game isn't over
	if (!isGameOver) {
		renderEntity(player);
	}
